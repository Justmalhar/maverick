...
describe("tauri command wrappers", () => {
  ...
  it("kanban / presets / mcp / misc", async () => {
    ...
    await api.autopilotList("p1");
    expect(invoke).toHaveBeenLastCalledWith("autopilot_list", { projectId: "p1" });
    await api.autopilotUpsert({ id: "a1" });
    expect(invoke).toHaveBeenLastCalledWith("autopilot_upsert", { autopilot: { id: "a1" } });
    await api.autopilotDelete("a1");
    expect(invoke).toHaveBeenLastCalledWith("autopilot_delete", { id: "a1" });
    await api.autopilotRunNow("a1");
    expect(invoke).toHaveBeenLastCalledWith("autopilot_run_now", { id: "a1" });
    await api.autopilotWebhookInfo();
    expect(invoke).toHaveBeenLastCalledWith("autopilot_webhook_info");
    await api.squadList("p1");
    expect(invoke).toHaveBeenLastCalledWith("squad_list", { projectId: "p1" });
    await api.squadUpsert({ id: "s1" });
    expect(invoke).toHaveBeenLastCalledWith("squad_upsert", { squad: { id: "s1" } });
    await api.squadDelete("s1");
    expect(invoke).toHaveBeenLastCalledWith("squad_delete", { id: "s1" });
